...
describe("calculateItemsInView", () => {
    ...
    describe("scroll optimization", () => {
        ...
        it("does not infer a deferred position delta from visible item expansion", () => {
            const previousPlatform = Platform.OS;
            Platform.OS = "android";
            try {
                ...
                expect(mockCtx.values.get("containerPosition1")).toBe(50);
                expect(mockCtx.values.get("containerPosition2")).toBe(100);
                expect(mockCtx.values.get("containerPosition3")).toBe(150);

                mockState.sizes.set("item_1", 150);
                ...
                expect(mockCtx.values.get("containerPosition1")).toBe(50);
                expect(mockCtx.values.get("containerPosition2")).toBe(200);
                expect(mockCtx.values.get("containerPosition3")).toBe(250);
            } finally {
                Platform.OS = previousPlatform;
            }
        });

        it("does not infer a deferred position delta when a partially visible row expands", () => {
            const previousPlatform = Platform.OS;
            Platform.OS = "android";
            try {
                mockState.props.data = Array.from({ length: 4 }, (_, i) => ({ id: i }));
                ...
                expect(mockCtx.values.get("containerItemKey2")).toBe("item_3");
                expect(mockCtx.values.get("containerPosition0")).toBe(50);
                expect(mockCtx.values.get("containerPosition1")).toBe(200);
                expect(mockCtx.values.get("containerPosition2")).toBe(250);
            } finally {
                Platform.OS = previousPlatform;
            }
        });

        it("does not infer a deferred position delta from a visible row near the end of mounted content", () => {
            const previousPlatform = Platform.OS;
            Platform.OS = "android";
            try {
                mockState.props.data = Array.from({ length: 4 }, (_, i) => ({ id: i }));
                ...
                mockState.sizes.set("item_2", 150);
                mockState.sizesKnown.set("item_2", 150);
                mockState.minIndexSizeChanged = 2;

                calculateItemsInView(mockCtx);

                expect(mockState.deferredPositionDelta).toBe(0);
                expect(mockCtx.values.get("containerItemKey0")).toBe("item_2");
                expect(mockCtx.values.get("containerItemKey1")).toBe("item_3");
        ...
        it("keeps deferred position state stable across repeated visible expansions while scrolling", () => {
            const previousPlatform = Platform.OS;
            Platform.OS = "android";
            try {
                mockState.props.data = Array.from({ length: 5 }, (_, i) => ({ id: i }));
                ...
                expect(mockState.positions[2]).toBe(200);
                expect(mockState.positions[3]).toBe(350);
                expect(mockState.positions[4]).toBe(400);
                expect(mockState.idsInView).toEqual(["item_1", "item_2"]);
            } finally {
                Platform.OS = previousPlatform;
            }
        });

        it("rebases cleanly on a direction-change boundary after a visible expansion during deferred scrolling", () => {
            const previousPlatform = Platform.OS;
            Platform.OS = "android";
            try {
                mockState.props.data = Array.from({ length: 15 }, (_, i) => ({ id: i }));
                ...
                mockState.sizes.set("item_11", 150);
                mockState.sizesKnown.set("item_11", 150);
                mockState.minIndexSizeChanged = 11;

                calculateItemsInView(mockCtx);

                expect(mockState.deferredPositionDelta).toBe(100);
                expect(mockState.positions[11]).toBe(650);
        ...
        it("rebases cleanly on a scroll-idle boundary after a visible expansion during deferred scrolling", () => {
            const previousPlatform = Platform.OS;
            Platform.OS = "android";
            try {
                mockState.props.data = Array.from({ length: 15 }, (_, i) => ({ id: i }));
        ...
        it("keeps multiple visible expansions local within the same mounted range", () => {
            const previousPlatform = Platform.OS;
            Platform.OS = "android";
            try {
                ...
                mockState.sizes.set("item_3", 120);
                mockState.sizesKnown.set("item_3", 120);
                mockState.minIndexSizeChanged = 3;
                ...
                expect(mockState.positions[1]).toBe(50);
                expect(mockState.positions[2]).toBe(200);
                expect(mockState.positions[3]).toBe(250);
                expect(mockState.positions[4]).toBe(370);
                expect(mockState.positions[5]).toBe(420);
            } finally {
                Platform.OS = previousPlatform;
            }
        });

        it("keeps a visible expansion local when a later above-viewport data change rebases deferred state", () => {
            const previousPlatform = Platform.OS;
            Platform.OS = "android";
            try {
                mockState.props.data = Array.from({ length: 5 }, (_, i) => ({ id: i }));
                ...
                expect(mockState.positions[3]).toBe(250);
                expect(mockState.positions[4]).toBe(300);
        ...
        it("can defer position-delta visual compensation while still using deferred delta detection", () => {
            const previousPlatform = Platform.OS;
            Platform.OS = "android";
            try {
                mockState.props.data = Array.from({ length: 3 }, (_, i) => ({ id: i }));
        ...
        it("disables and resets deferred position state while an imperative scroll target is active", () => {
            ...
            try {
                ...
                mockState.deferredPositionBaseline.set("item_0", 0);
        ...
        it("keeps data-change passes rebased after an imperative scroll target settles", () => {
            const previousPlatform = Platform.OS;
            Platform.OS = "android";
            try {
                mockState.props.data = Array.from({ length: 3 }, (_, i) => ({ id: i }));
        ...
        it("does not seed a new deferred position baseline during forceFullItemPositions passes", () => {
            const previousPlatform = Platform.OS;
            Platform.OS = "web";
            try {
                mockState.props.data = Array.from({ length: 3 }, (_, i) => ({ id: i }));
                ...
                expect(mockState.deferredPositionBaseline.size).toBe(3);
        ...
        it("rebases the pending deferred position delta on data-change passes even when deferred mode is supported", () => {
            const previousPlatform = Platform.OS;
            Platform.OS = "android";
            try {
                mockState.props.data = Array.from({ length: 3 }, (_, i) => ({ id: i }));
