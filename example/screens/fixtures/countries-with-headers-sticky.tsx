...
const createDataWithHeaders = (
    countriesData: (Country | BigCountry)[],
): { data: ListItem[]; headerIndices: number[] } => {
    ...
    Object.keys(grouped)
        .sort()
        .forEach((letter) => {
            // Add header and track its index for sticky functionality
    ...
    return { data: result, headerIndices };
};

const { data: DATA_WITH_HEADERS, headerIndices: HEADER_INDICES } = createDataWithHeaders(COUNTRIES);
