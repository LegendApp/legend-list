...
import { ENABLE_DOM_REORDER } from "@/constants";
...
export function useDOMOrder() {
    ...
    useEffect(() => {
        if (Platform.OS !== "web" || !ENABLE_DOM_REORDER) {
            return;
        }
        ...
        const unsubscribe = listen$(ctx, "lastPositionUpdate", () => {
            ...
            debounceRef.current = setTimeout(() => {
                ...
                const firstRef = ctx.viewRefs.get(0);
                if (firstRef?.current) {
                    const parent = firstRef.current.parentElement;
                    if (parent) {
                        sortDOMElementsPatience(parent as HTMLDivElement);
                    }
                }
