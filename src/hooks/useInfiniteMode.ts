...
interface InfiniteModeProps<T> {
    ...
    renderItem: (props: LegendListRenderItemProps<T, any>) => React.ReactNode;
...
export function useInfiniteMode<T, TProps extends InfiniteModeProps<T>>(
    ...
    if (IS_DEV && enabled) {
        if ((props.numColumns ?? 1) > 1) {
            warnDevOnce("infiniteMode-numColumns", "infiniteMode does not support numColumns > 1.");
        }
        if (props.onEndReached || props.onStartReached) {
            warnDevOnce(
                "infiniteMode-reached",
                "onStartReached/onEndReached are ignored with infiniteMode because the list has no edges.",
            );
        }
        if (props.ListHeaderComponent || props.ListFooterComponent) {
            warnDevOnce(
                "infiniteMode-header-footer",
                "ListHeaderComponent/ListFooterComponent are not supported with infiniteMode and can break scroll recentering.",
            );
        }
        if (props.maintainScrollAtEnd) {
            warnDevOnce("infiniteMode-maintainScrollAtEnd", "maintainScrollAtEnd is not supported with infiniteMode.");
        }
    }
