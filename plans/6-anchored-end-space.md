...
- [ ] Add the core `anchoredEndSpace` primitive, dedicated internal state, and core tests for size calculation/clearing/merge behavior.
- [ ] Migrate `keyboard-chat` to use `anchoredEndSpace` and add native adapter coverage without using `reportContentInset`.
